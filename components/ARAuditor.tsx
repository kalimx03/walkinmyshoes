            ...
            {isScanning && !isLiveMode && (
              <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-40">
                <button 
                  ...
                  disabled={loading}
                  className="relative w-32 h-32 group pointer-events-auto"
                >
                  <div className={`absolute inset-0 rounded-full border-8 border-indigo-500/20 ${loading ? 'animate-ping' : 'group-hover:scale-125 transition-transform duration-500'}`}></div>
                  <div className={`w-full h-full rounded-full bg-white flex items-center justify-center shadow-2xl relative z-10 transition-all ${loading ? 'scale-90 opacity-60' : 'active:scale-95'}`}>
